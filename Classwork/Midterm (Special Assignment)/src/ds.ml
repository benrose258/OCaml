...
type exp_val =
  ...
  | TableVal of (exp_val*exp_val) list
and
  env =
  | EmptyEnv
...
let table_of_tableVal : exp_val -> ((exp_val*exp_val) list) ea_result = function
  | TableVal l -> return l
  | _ -> error "Expected a hash table!"
