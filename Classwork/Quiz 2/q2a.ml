...
(* let rec prune_helper prune_level current_level tree =
  match tree with
  | Empty -> Empty
  | Node(root,lt,rt) ->
    if (current_level = prune_level)
    then Empty
    else Node(root,prune_helper prune_level (current_level + 1) lt, prune_helper prune_level (current_level + 1) rt) *)
...
let rec prune n t =
  ...
  | Node(root,lt,rt) ->
    if (n = 0)
    then Empty
    else Node(root, prune (n-1) lt, prune (n-1) rt)
...
let rec prune n t =
  ...
  | (_,Empty) -> Empty
  | (m,Node(d,lt,rt)) ->
       Node(d,prune (m-1) lt, prune (m-1) rt)
