...
(* My coded record_of_recordVal function *)
let record_of_recordVal = function
  | RecordVal r -> return r
  | _ -> error "Expected a record!"

(* My coded function to convert a RefVal in exp_val form to its integer reference number *)
let int_of_expRefVal : exp_val -> int ea_result =  function
  | RefVal n -> return n
  | _ -> error "Expected a reference!"
