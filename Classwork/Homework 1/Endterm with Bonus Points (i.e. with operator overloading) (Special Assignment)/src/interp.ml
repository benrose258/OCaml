...
(* Backend Operator Overloading Function: Name_Mangle *)
(* Add extra characters in the backend to the name of an overloaded function
                          to differentiate between multiple duplicate functions in the frontend. *)

(* Overview of variables:
    n: Name of the string we're mangling. For example, in name_mangle, the variable "n" would be equal to name_mangle.
    es: The variables that the function has as its input. For example, in name_mangle, they would be "n" and "es". *)
let name_mangle n es =
  (* Return the string created by concatenating the following:
      the string n,
      concatenating with "_",
      and then concatenating that entire string above with
            the string version of the number of arguments provided to the source function,
            i.e. what the length of the list of variables is *)
  n^"_"^string_of_int (List.length es)
  (* Testing modified name_mangle for initialize below here: *)
  (* if (n = "initialize")
  then "initialize"
  else n^"_"^string_of_int (List.length es) *)
...
let rec get_methods cs c_name fss = function
  ...
  | Class (name,super,_fields,methods)::_ when name=c_name ->
    (List.map (fun (Method(n,pars,body))
               (* Original (before mangling the name) is in the comment below *)
                (* -> (n,(pars,body,super,List.flatten fss))) *)

               (* Newly modified line, after mangling the name n *)
               -> ((name_mangle n pars),(pars,body,super,List.flatten fss)))
...
let lookup_class : string -> class_env -> class_decl ea_result  = fun c_name c_env ->
  ...
  | Some (super,fields,methods) ->
    return (super,fields,methods)
...
let rec apply_method : string -> exp_val -> exp_val list -> method_decl -> exp_val ea_result =
...
and
  ...
  | NewObject(c_name,es) ->
    ...
    (* Original line before name_mangle is directly below this comment *)
    (* in (match List.assoc_opt "initialize" methods with *)

    (* New line with name_mangle *)
  ...
  | Send(e,m_name,es) ->
    ...
    (* l
    | None -> error "send Method not found"
    *)
    (* Original line before name_mangle is directly below this comment *)
